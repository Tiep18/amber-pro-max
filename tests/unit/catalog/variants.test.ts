...
const {requireAdmin, createSupabaseServerClient, revalidatePath} = vi.hoisted(() => ({
  requireAdmin: vi.fn(),
...
import {inventoryAdjustmentSchema, variantDraftSchema, variantPriceOverrideSchema} from '@/catalog/variant-schemas';
...
function createFromMock() {
  ...
  const from = vi.fn((table: string) => ({
    ...
    insert: vi.fn((payload: unknown) => {
      ...
      return {select: () => ({single: () => Promise.resolve({data: {id: variantId}, error: null})})};
    }),
...
describe('effective variant pricing', () => {
  it('uses variant override before parent fallback while preserving market currency', () => {
    const price = resolveEffectiveVariantPrice({
      ...
      variantOverrides: [{marketCode: 'vn', enabled: true, currencyCode: 'VND', priceMinor: 150000}]
    });
...
describe('variant actions', () => {
  ...
  it.each([
    ['save variant', () => saveVariantAction({productId, variantId, sku: 'BEAR-S', attributes: '{"size":"small"}', displayOrder: 0, mediaId: null})],
    ['price override', () => saveVariantPriceOverrideAction({variantId, marketCode: 'intl', currencyCode: 'USD', priceMinor: 900})],
    ['inventory', () => adjustInventoryAction({ownerType: 'variant', productId, variantId, quantityOnHand: 3})],
  ...
  it('rejects variant payloads for PDF products before writing', async () => {
    const productQuery = {
      ...
      maybeSingle: vi.fn(() => Promise.resolve({data: {id: productId, product_type: 'pdf_pattern'}, error: null}))
    };
  ...
  it('upserts explicit variants and maps duplicate SKU database failures', async () => {
    const productQuery = {
      select: vi.fn(() => productQuery),
      eq: vi.fn(() => productQuery),
      maybeSingle: vi.fn(() => Promise.resolve({data: {id: productId, product_type: 'physical_finished'}, error: null}))
    };
  ...
  it('stores product-level inventory only for non-variant physical products', async () => {
    ...
    const result = await adjustInventoryAction({ownerType: 'product', productId, quantityOnHand: 5});
