...
test('exception request page is available without reserving inventory', async ({page}) => {
  ...
  await expect(
    page.getByRole('heading', {name: 'Ask the studio to review a special checkout case'})
  ).toBeVisible();
  ...
  await expect(page.getByText('No inventory is reserved by this request')).toBeVisible();
});
