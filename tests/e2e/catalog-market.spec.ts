...
for (const combination of combinations) {
  test(`catalog projects ${combination.locale}+${combination.market} with matching currency`, async ({
    ...
    const session = await storefrontMarket.createSession({
      ...
      marketCookie: combination.market
    });
    ...
    const grid = session.page.getByTestId('catalog-product-grid');
    const cards = grid.getByRole('article');
    expect(await cards.count()).toBeGreaterThan(0);
    await expect(grid).toContainText(combination.currency, { timeout: 15_000 });
    for (const card of await cards.all()) {
      const text = await card.textContent();
      if (text && /₫|VND|\$|USD/.test(text)) {
        expect(text).toMatch(combination.currency);
      }
    }
...
test('desktop exposes independent semantic language and shopping-region groups', async ({
  ...
  await header.getByTestId('commerce-context-trigger').click();
...
test('mobile exposes independent 44px language and shopping-region choices', async ({
  ...
  for (const option of await language.getByRole('link').all()) {
