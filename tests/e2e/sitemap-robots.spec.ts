...
test('SEO-02 SEO-04 D-05 D-07 sitemap index points to localized public sitemaps only', async ({page}) => {
  ...
  expect(body).toContain('http://127.0.0.1:3210/sitemaps/en');
  expect(body).toContain('http://127.0.0.1:3210/sitemaps/vi');
...
test('SEO-04 localized sitemap includes public catalog URLs and excludes private surfaces', async ({page}) => {
  ...
  expect(body).toContain('http://127.0.0.1:3210/en/catalog');
  expect(body).toContain('http://127.0.0.1:3210/en/product/both-market-bear');
...
test('SEO-04 robots disallows private and operational surfaces', async ({page}) => {
  ...
  expect(body).toContain('Sitemap: http://127.0.0.1:3210/sitemap.xml');
