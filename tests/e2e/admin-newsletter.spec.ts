...
test.describe('admin newsletter subscribers (NEWS-03, D-15, D-16)', () => {
  ...
  test('renders dense read-only subscriber rows with filters', async ({ page }) => {
    ...
    await page.goto(
      `/admin/newsletter?status=subscribed&locale=en&market=intl&search=${encodeURIComponent(seed.newsletterReaderEmail.slice(0, 8))}`
    );
    await expect(page.getByRole('heading', { name: /newsletter subscribers/i })).toBeVisible();
    await expect(page.getByLabel('Search email')).toHaveValue(
      seed.newsletterReaderEmail.slice(0, 8)
    );
  ...
  test('does not expose consent override controls or raw metadata', async ({ page }) => {
    ...
    await expect(
      page.getByText(/token_hash|raw token|raw ip|user_agent_hash|[a-f0-9]{64}/i)
    ).toHaveCount(0);
  });
});
