...
const usRegionCodeSchema = z
  .string()
  .trim()
  .toUpperCase()
  .regex(
    /^(?:AL|AK|AS|AZ|AR|CA|CO|CT|DE|DC|FL|GA|GU|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MH|MA|MI|FM|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|MP|OH|OK|OR|PW|PA|PR|RI|SC|SD|TN|TX|UT|VT|VI|VA|WA|WV|WI|WY)$/
  );

const shippingProfileSaveSchema = z
  .object({
...
const shippingRuleSaveSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.destinationKind === 'exact_country' && !value.countryCode) {
      context.addIssue({
        code: 'custom',
        path: ['countryCode'],
        message: 'An exact rule needs a country.'
      });
    }
    if (value.destinationKind === 'fallback' && value.countryCode) {
      context.addIssue({
        code: 'custom',
        path: ['countryCode'],
        message: 'A fallback rule has no country.'
      });
    }
  });

const shippingRegionAdjustmentSaveSchema = z
  .object({
...
export type ShippingAdminActionResult =
  ...
  | {
      status: 'invalid';
      code:
        | 'invalid_shipping_profile'
        | 'invalid_shipping_rule'
        | 'invalid_shipping_region'
        | 'duplicate_shipping_rule'
        | 'duplicate_shipping_region';
    }
  | {
      status: 'error';
      code:
        | 'shipping_profile_failed'
        | 'shipping_rule_failed'
        | 'shipping_region_failed'
        | 'shipping_default_failed';
    };
...
export async function createShippingProfileAction(
  formData: FormData
): Promise<CreateShippingProfileResult> {
  await requireAdmin();
  ...
  const additionalItemFeeMinor = moneyToMinor(
    parsed.data.additionalItemFee,
...
export async function deactivateShippingProfileAction(
  profileId: string
): Promise<DeactivateShippingProfileResult> {
  return setShippingProfileActiveAction(profileId, false);
}

export async function setShippingProfileActiveAction(
  profileId: string,
  active: boolean
): Promise<DeactivateShippingProfileResult> {
  await requireAdmin();
  const parsed = deactivateShippingProfileSchema.safeParse({ profileId, active });
  if (!parsed.success) {
    return { status: 'invalid', code: 'invalid_shipping_profile' };
  }

  let failurePhase: 'shipping_profile_deactivate' | 'shipping_rule_deactivate' =
    'shipping_profile_deactivate';

  return runMonitoredAction({
    area: 'admin',
    action: parsed.data.active ? 'shipping_reactivate' : 'shipping_deactivate',
    errorCode: parsed.data.active ? 'shipping_reactivate_failed' : 'shipping_deactivate_failed',
    summary: parsed.data.active
      ? 'Admin shipping reactivation failed'
      : 'Admin shipping deactivation failed',
    facts: {
    ...
    operation: async () => {
      const supabase =
        (await createSupabaseServerClient()) as unknown as ShippingAdminDatabaseClient;
      if (!parsed.data.active) {
        const { data: defaultRow, error: defaultError } = await supabase
          .from('shipping_store_defaults')
          .select('id')
          .eq('shipping_profile_id', parsed.data.profileId)
          .eq('active', true)
          .maybeSingle();
        if (defaultError) {
        ...
        if (defaultRow) {
          return { status: 'invalid', code: 'default_shipping_profile' } as const;
        }
      }
...
export async function saveShippingProfileAction(
  input: unknown
): Promise<ShippingAdminActionResult> {
  await requireAdmin();
...
export async function setStoreDefaultShippingProfileAction(
  profileId: string
): Promise<ShippingAdminActionResult> {
  await requireAdmin();
  ...
  return adminShippingMutation({
    ...
    operation: async () => {
      const supabase =
        (await createSupabaseServerClient()) as unknown as ShippingAdminDatabaseClient;
      const { error } = await supabase.rpc('admin_set_shipping_store_default', {
        p_profile_id: parsed.data
      });
      return error
        ? ({ status: 'error', code: 'shipping_default_failed' } as const)
        : ({ status: 'updated' } as const);
    }
  }).then((result) => {
      ...
      if (parsed.data.ruleId) {
        ...
        return error
          ? isUniqueViolation(error)
            ? ({ status: 'invalid', code: 'duplicate_shipping_rule' } as const)
            : ({ status: 'error', code: 'shipping_rule_failed' } as const)
          : ({ status: 'updated' } as const);
      }
      const { data, error } = await supabase
        .from('shipping_rules')
...
export async function saveShippingRegionAdjustmentAction(
  input: unknown
): Promise<ShippingAdminActionResult> {
  await requireAdmin();
  ...
  return adminShippingMutation({
    ...
    operation: async () => {
      ...
      if (parsed.data.adjustmentId) {
        const { error } = await supabase
          .from('shipping_region_adjustments')
          .update(values)
          .eq('id', parsed.data.adjustmentId);
        return error
          ? isUniqueViolation(error)
            ? ({ status: 'invalid', code: 'duplicate_shipping_region' } as const)
            : ({ status: 'error', code: 'shipping_region_failed' } as const)
          : ({ status: 'updated' } as const);
      }
      const { data, error } = await supabase
        .from('shipping_region_adjustments')
