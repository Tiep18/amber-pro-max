  ...
  return {
    ...
    batchSize:
      body && typeof body === 'object'
        ? parseBatchSize((body as { batchSize?: unknown }).batchSize)
        : undefined
  };
}
...
export async function POST(request: Request) {
  ...
  if (
    !env.transactionalEmailWorkerSecret ||
    bearerSecret(request) !== env.transactionalEmailWorkerSecret
  ) {
  ...
  const { createProductionEmailOutboxRepository, createResendTransactionalEmailSender } =
    await import('@/fulfillment/email-outbox.server');
