...
import {confirmVietQrPaymentAction, rejectVietQrPaymentAction, type VietQrAdminActionResult} from '@/payments/admin-actions';
...
function ResultMessage({state}: {state: FormState}) {
  ...
  const tone = state.status === 'confirmed' || state.status === 'rejected' ? 'text-[var(--success)]' : 'text-[var(--destructive)]';
  return (
    <p role="alert" className={`rounded-[var(--radius-card)] bg-[var(--surface-muted)] p-3 text-sm font-semibold ${tone}`}>
      Result: {state.status}
...
export function VietQrEvidenceForm({order}: {order: AdminOrderDetail}) {
  ...
  return (
    <section aria-labelledby="vietqr-decision-heading" className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-4">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <form action={confirmFormAction} className="grid gap-3 rounded-[var(--radius-card)] bg-[var(--surface-muted)] p-3">
          <input type="hidden" name="orderId" value={order.orderId} />
          <input type="hidden" name="idempotencyKey" value={idempotencyKey(order.orderNumber, 'confirm')} />
          <label className="grid gap-1 text-sm font-semibold">
            Bank reference
            <input name="bankReference" required defaultValue={evidence.transferReference} className="min-h-11 rounded-[var(--radius-control)] border border-[var(--border)] px-3" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Received amount
            <input
          ...
          <label className="grid gap-1 text-sm font-semibold">
            Received at
            <input name="receivedAt" required type="datetime-local" className="min-h-11 rounded-[var(--radius-control)] border border-[var(--border)] px-3" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Private receipt path
            <input name="privateReceiptPath" placeholder="private/vietqr/receipt.jpg" className="min-h-11 rounded-[var(--radius-control)] border border-[var(--border)] px-3" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Admin note
            <textarea name="adminNote" className="min-h-24 rounded-[var(--radius-control)] border border-[var(--border)] px-3 py-2" />
          </label>
          <Button type="submit" disabled={!evidence.actionAvailable || confirmPending} aria-disabled={!evidence.actionAvailable || confirmPending}>
            {confirmPending ? 'Confirming payment' : 'Confirm payment'}
          </Button>
          <ResultMessage state={confirmState} />
        </form>

        <form action={rejectFormAction} className="grid gap-3 rounded-[var(--radius-card)] bg-[var(--surface-muted)] p-3">
          <input type="hidden" name="orderId" value={order.orderId} />
          <input type="hidden" name="idempotencyKey" value={idempotencyKey(order.orderNumber, 'reject')} />
          <label className="grid gap-1 text-sm font-semibold">
            Reject reason
            <input name="reason" required placeholder="amount_mismatch" className="min-h-11 rounded-[var(--radius-control)] border border-[var(--border)] px-3" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Decision note
