  ...
  return (
    <div className="grid gap-4">
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        />
        <AdminMetricCard label="Ready" value={readyCount} description="passing checks" />
        <AdminMetricCard label="Blocked" value={blockedCount} description="needs evidence" />
      </section>

      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            <ListChecks className="size-5 text-[var(--accent)]" aria-hidden="true" />
            <CardTitle>Launch readiness checklist</CardTitle>
          </div>
          <p className="text-sm text-[var(--muted-foreground)]">
            Store enabled countries, tax stance, seller decisions, provider/manual UAT evidence, E2E
            evidence, and monitoring/redaction readiness as fail-closed launch gates.
          </p>
        </CardHeader>
        <CardContent className="grid gap-6">
          <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
            {readiness.gates.map((gate) => (
              <div key={gate.id} className="grid gap-3 py-4 sm:grid-cols-[auto_1fr_auto]">
                {gate.status === 'ready' ? (
                  <CheckCircle2 className="mt-1 size-5 text-[var(--success)]" aria-hidden="true" />
                ) : (
          ...
          <form action={saveLaunchSettingsAction} className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <textarea
                name="taxStance"
                defaultValue={settings.taxStance ?? ''}
                className={fieldClass()}
                rows={3}
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <textarea
                name="e2eEvidence"
                defaultValue={settings.e2eEvidence ?? ''}
                className={fieldClass()}
                rows={3}
              />
            </label>
