...
import type {AdminOperationalError, AdminOperationalErrorFilters} from '@/operations/admin-queries';
...
function filterHref(filters: Required<AdminOperationalErrorFilters>, next: AdminOperationalErrorFilters) {
...
function factEntries(error: AdminOperationalError) {
  return Object.entries(error.sanitizedFacts).map(([key, value]) => [key, typeof value === 'object' ? JSON.stringify(value) : String(value)] as const);
}
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-[var(--muted-foreground)]">
          Sanitized operational error review. Raw provider payloads, signatures, secrets, tokens, signed URLs, full addresses, and unnecessary PII are not shown.
        </p>
      ...
      <CardContent>
        {errors.length === 0 ? (
          <div className="grid gap-1 text-[var(--muted-foreground)]">
            <p className="font-semibold text-[var(--foreground)]">No unresolved operational errors</p>
            <p>New sanitized errors will appear here when they need review.</p>
          </div>
        ) : (
          <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
            {errors.map((error) => (
              <div key={error.id} className="grid gap-4 py-4 lg:grid-cols-[1fr_auto]">
                <div className="grid gap-3">
                  <div>
                    <p className="text-sm font-semibold uppercase text-[var(--accent)]">
                      {error.area} / {error.severity} / {error.status}
                    </p>
