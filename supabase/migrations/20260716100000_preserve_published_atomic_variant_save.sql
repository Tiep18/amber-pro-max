-- Evaluate publication eligibility after the complete variant aggregate is
-- stored, so the variant-insert trigger cannot make a transient missing-
-- inventory state permanent. A genuinely invalid final aggregate remains
-- safely demoted by the existing catalog invariant.
